...
class EventService:
        ...
        *,
        ...
        if bundle.intent_ids and self._investigation_intent is not None:
            self._investigation_intent.schedule_dispatch(
                event_id=bundle.event.event_id,
                intent_id=bundle.intent_ids[0] if len(bundle.intent_ids) == 1 else None,
                trigger="ingest",
            )
    ...
    async def merge_report_quality_context_snapshot(
        ...
        """Persist ``report_quality`` onto the durable snapshot (ISSUE-348)."""
        from app.services.event_context_snapshot_projection import (
        ...
        try:
            async with self._session_factory() as session:
                async with session.begin():
                    ...
                    if row is None:
                        return
                    row.event_context_snapshot = merge_report_quality_into_snapshot(
                        (
                            dict(row.event_context_snapshot)
                            if isinstance(row.event_context_snapshot, dict)
                            else None
            ...
            logger.warning(
                "merge_report_quality_context_snapshot failed event_id=%s",
                event_id,
                exc_info=True,
            )
            await self._mark_snapshot_merge_degraded(
                event_id, reason=f"report_quality:{type(exc).__name__}"
            )
