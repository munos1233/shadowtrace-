...
# Scenario-aligned default window when triage has no explicit time range.
...
class EvidenceRepository(Protocol):
    """Persistence port for Evidence rows (conflict = evidence_id)."""
...
class EvidenceAgent(BaseAgent[EvidenceAgentInput, EvidenceOutput]):
    ...
    def __init__(
        ...
        *,
        ...
        # Populated each run for audit / acceptance checks.
    ...
    async def _run(self, input: EvidenceAgentInput) -> EvidenceOutput:
        ...
        for tool_name in EVIDENCE_QUERY_ORDER:
            ...
            await self._note_timing(
                ...
                tool_name,
                status=(
                    tool_result.status.value if tool_result is not None else f"error:{call_error}"
                ),
                execution_time_ms=timing_ms,
            )
    ...
    def _resolve_time_range(self, _input: EvidenceAgentInput) -> dict[str, str]:
        # TriageResult has no time_range field (ISSUE-005); use agent default window.
    ...
    def _build_params(
        ...
        *,
        ...
        account = next(
            (a.username for a in entities.accounts if a.username),
            None,
        )
    ...
    def _dedup_and_sort(items: list[Evidence]) -> list[Evidence]:
        ...
        for item in items:
            ...
            if existing is None or item.confidence > existing.confidence:
                # Keep truncated timestamp on the winning row for stable sort.
