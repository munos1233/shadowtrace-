"""Shared fixtures for tool-system tests (ISSUE-025).

Provides isolated mock state cleanup and deterministic executor wiring so
integration-style tool chains can run without real Redis/PostgreSQL.
Available to both ``tests/test_tools/`` and ``tests/integration/``.
"""
