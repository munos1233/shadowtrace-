...
class _FakeWorkingMemory:
    """Minimal BoundWorkingMemory stand-in for EvidenceAgent writes."""
...
async def test_all_seven_sources_completed_timeline_and_persistence(
    ...
    """主场景：≥5 个成功源、时间线单调递增、落库与 Context 一致。"""
    ...
    # Per-query timings recorded for agent_trace / scratchpad acceptance.
...
async def test_three_tool_failures_partial_done_penalty(
    ...
    """部分失败：成功源 3–4 → partial_done，置信度惩罚 0.10。

    ISSUE 文案写「2 路失败」，但 status 表要求成功源 3–4 才是 partial_done；
    在 7 路中强制失败 3 路（剩余 4 路成功）以匹配判定规则。
    """
    ...
    # Recompute confidence without penalty and verify 0.10 deduction.
...
async def test_all_tools_failed_returns_failed_without_raise(
    ...
    """全部失败：collection_status=failed，不抛异常。"""
...
async def test_dedup_keeps_higher_confidence_and_sorts_by_timestamp() -> None:
    """去重键 (source, evidence_type, timestamp) 保留更高 confidence，并按秒级升序。"""
...
async def test_evidence_table_count_matches_list_after_upsert(
    ...
    """evidence 表（仓库）写入条数与 evidence_list 一致。"""
